...
- 新用户反馈：当前远征已经可玩，但悬浮平台与简单背景仍有明显 Demo 感，希望地形像一个真实、完整、可沉浸探索的世界。
- 本轮目标：保留现有主线、存档与自动化坐标兼容，扩建寂羽空洞的支路和终局区域，并用连续岩体、建筑遗迹、区域光色、前景遮挡、环境动效与叙事地标取代“纯平台串联”的观感。
- 地图宽度从 3200 扩为 4200；规划新增遗忘档案窟、沉钟蓄水池、天线墓园三个区域，并把原有前庭、裂谷、机房组织成有回环的房间网络。
- 已抽出 `worldLayout.ts` 统一描述 42 段地形、三条路线、七个房间与四处环境叙事回声；主线旧坐标不变，新增左侧档案回环、右下蓄水池回环和 Boss 后墓园延伸。
- 场景加入分区岩体/建筑平台、前庭拱廊、竖井井壁、地下菌林、蓄水池钟架、机房转子、墓园天线，补充区域光场、水纹、孢子、前景岩层、暗角和进区标题演出。
- 新增档案窟、蓄水池、墓园敌人和补给箱；新增沉钟与墓园撤离点；四处叙事回声可按 E 聆听。地图页同步展示七个房间和三条可行路线。

## 2026-07-17 · 实体悬崖与高跳重构

- 用户复看截图后指出：细长平台仍然过密，观感接近 Doodle Jump；希望提高跳跃、减少平台，并用有明确体积的地面、悬崖和边界替代可穿透跳板。
- 所有主要地形已从单向穿透平台改为完整静态碰撞体；角色不能从下方穿过，必须绕过可见侧壁、从悬崖边缘起跳后落到顶面。碰撞体与绘制岩体使用同一组边界。
- 地形段从扩建初稿的 42 段压缩为 21 段；回声竖井只保留三座交错的实体岩壁并直接接入荧菌裂谷，主路径不再依靠连续小跳板。
- 竖井岩体深度扩到 240–300 像素，让平台表现为上下层相互咬合的岩壁和断崖；前庭保留连续地面，机房改为完整竞技场基座，墓园为世界边界陆块。
- 满跳初速度从 -650 提高到 -1050，并保留松开跳跃键后的短跳截断；水平空中控制与起跳缓冲同步调整，使更大的高度差仍可控。
